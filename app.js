/* Training Arc OS v9 Ulti — local-first encrypted all-in-one tracker */
const APP_KEY = "training_arc_os_v9_vault";
const SETTINGS_KEY = "training_arc_os_v9_settings";
const LEGACY_KEYS = ["training_arc_os_v8_vault","training_arc_os_v7_vault","training_arc_os_v6_vault","training_arc_os_v5_vault","training_arc_os_v4_vault","training_arc_os_v3_vault","training_arc_os_v2_vault"];
...
function bindEvents(){
  ...
  $("createVaultBtn").onclick=async()=>{ const pass=$("vaultPassword").value; if(!pass||pass.length<6)return toast("Pro v9 dej aspoň 6 znaků."); if(getStoredVault()&&!confirm("Přepsat existující vault?"))return; state=seedState(); vaultPassword=pass; unsafeMode=false; await saveVault(false); openApp(); toast("V9 secure vault vytvořen."); };
  ...
  $("resetVaultBtn").onclick=()=>{ if(confirm("Smazat lokální data v9 + legacy keys?")){ localStorage.removeItem(APP_KEY); localStorage.removeItem(APP_KEY+"_unsafe"); LEGACY_KEYS.forEach(k=>localStorage.removeItem(k)); location.reload(); }};
...
async function exportEncrypted(){ if(!encryptedVaultCache) encryptedVaultCache=await encryptState(state,vaultPassword); download(`training_arc_v9_encrypted_${todayIso()}.json`,JSON.stringify(encryptedVaultCache,null,2)); }
function exportPlain(){ if(!confirm("Plain export není šifrovaný. Fakt stáhnout?"))return; download(`training_arc_v9_plain_${todayIso()}.json`,JSON.stringify(state,null,2)); }
...
function exportIcs(){
  const lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Training Arc OS v9//CZ'];
